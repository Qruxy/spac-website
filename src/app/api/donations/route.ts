...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (captureResult.status === 'COMPLETED') {
      ...
      // userId is required on Payment; only persist if the donor is logged in.
      // Anonymous donations are still captured by PayPal — order_id in the redirect URL provides traceability.
      if (session?.user?.id) {
