...
type MemberType    = 'INDIVIDUAL' | 'FAMILY' | 'PATRON' | 'BENEFACTOR';
type MemberInterval = 'MONTHLY' | 'ANNUAL';
...
interface PaymentRecord {
  ...
  user: {
    ...
    name:      string | null;
    email:     string;
    membership: { type: MemberType; interval: MemberInterval | null } | null;
  };
  registrations: Array<{
    id:    string;
...
const PLAN_LABELS: Record<MemberType, string> = {
...
const STATUS_CONFIG: Record<PaymentStatus, { label: string; color: string; icon: React.ReactNode }> = {
  SUCCEEDED:          { label: 'Succeeded',   color: 'text-green-500  bg-green-500/10',   icon: <CheckCircle className="h-3 w-3" /> },
  REFUNDED:           { label: 'Refunded',    color: 'text-blue-400   bg-blue-400/10',    icon: <RotateCcw   className="h-3 w-3" /> },
  PARTIALLY_REFUNDED: { label: 'Partial Ref', color: 'text-yellow-400 bg-yellow-400/10',  icon: <RotateCcw   className="h-3 w-3" /> },
  PENDING:            { label: 'Pending',     color: 'text-gray-400   bg-gray-400/10',    icon: <Clock       className="h-3 w-3" /> },
  FAILED:             { label: 'Failed',      color: 'text-red-500    bg-red-500/10',     icon: <XCircle     className="h-3 w-3" /> },
};

// ── Build a clear human-readable description ──────────────────────────────────
function buildDetails(p: PaymentRecord): { primary: string; secondary: string | null } {
  const plan = p.user.membership;
  const planLabel = plan
    ? `${PLAN_LABELS[plan.type] ?? plan.type}${plan.interval ? ' · ' + (plan.interval === 'ANNUAL' ? 'Annual' : 'Monthly') : ''}`
    : null;
  ...
  switch (p.type) {
    case 'SUBSCRIPTION':
      return {
        ...
        secondary: planLabel ?? p.description ?? null,
      };
    case 'EVENT_TICKET': {
      const evt = p.registrations[0]?.event;
      return {
        primary:   evt?.title ?? 'Event Ticket',
        secondary: evt ? null : (p.description ?? null),
      };
    }
    case 'OBS_REGISTRATION':
      return {
        primary:   'OBS Registration',
        secondary: p.registrations[0]?.event?.title ?? p.description ?? null,
      };
    case 'CAMPING_FEE':
      return {
        primary:   'Camping Fee',
...
export default function AdminPaymentsPage() {
  ...
  async function submitRefund() {
    ...
    try {
      ...
      setRefundModal(null);
      setRefundSuccess(
        `Refund of ${fmtAmount(String(data.refundAmount), refundModal.payment.currency)} issued (ID: ${data.refundId})`
      );
  ...
  const canRefund = (p: PaymentRecord) =>
    (['SUCCEEDED', 'PARTIALLY_REFUNDED'] as PaymentStatus[]).includes(p.status) && !!p.paypalCaptureId;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {total} records · {fmtAmount(String(totalAmount), 'USD')} filtered total
            </p>
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                ...
                <th className="text-left px-4 py-3 font-medium text-muted-foreground">Payment Details</th>
                ...
                payments.map(p => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      {/* Payment Details — type + rich context */}
                      <td className="px-4 py-3 max-w-[260px]">
                        <p className="font-medium text-foreground truncate" title={details.primary}>
                          {details.primary}
                        </p>
                        {details.secondary && (
                          <p className="text-xs text-muted-foreground truncate mt-0.5" title={details.secondary}>
                            {details.secondary}
                          </p>
                        )}
                      </td>

                      {/* Source — PayPal badge + link */}
                      <td className="px-4 py-3">
                        {p.paypalOrderId ? (
                          <a
                            href={`https://www.paypal.com/activity/payment/${p.paypalOrderId}`}
                            target="_blank" rel="noopener noreferrer"
                            className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 transition-colors text-xs font-medium"
                            title={`PayPal Order: ${p.paypalOrderId}`}
                          >
                      ...
                      <td className="px-4 py-3 text-right">
                        {canRefund(p) && (
                          <button onClick={() => openRefund(p)}
                            className="flex items-center gap-1.5 px-2.5 py-1 rounded text-xs bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 transition-colors ml-auto">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-card border border-border rounded-2xl shadow-2xl p-6 space-y-5">
            ...
            <div className="rounded-lg bg-muted/50 border border-border p-4 space-y-1.5 text-sm">
              {(() => {
                const d = buildDetails(refundModal.payment);
                return (
                  <>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-foreground text-right max-w-[200px]">
                        {d.primary}{d.secondary ? ` — ${d.secondary}` : ''}
                      </span>
                    </div>
                    <div className="flex justify-between">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input type="number" step="0.01" min="0.01" max={refundModal.payment.amount}
            ...
            <div className="space-y-1.5">
              ...
              <input type="text" maxLength={255} placeholder="e.g. Event cancelled, full refund issued"
            ...
            {refundModal.error && (
              <div className="flex items-center gap-2 rounded-lg bg-red-500/10 border border-red-500/20 px-3 py-2.5 text-sm text-red-400">
                <AlertCircle className="h-4 w-4 shrink-0" /> {refundModal.error}
              </div>
            )}

            <div className="flex gap-3 pt-1">
              <button onClick={() => setRefundModal(null)} disabled={refundModal.loading}
              ...
              <button onClick={submitRefund}
                ...
                {refundModal.loading
                  ...
                  : <><RotateCcw className="h-4 w-4" />Refund {fmtAmount(refundModal.amount || '0', refundModal.payment.currency)}</>
                }
              </button>
            </div>
          </div>
