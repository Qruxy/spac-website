 ...
 * BlurText Component
 *
 * Animated text that blurs in character by character or word by word.
...
export function BlurText({
  ...
  useEffect(() => {
    ...
    // Check if element is already in view (handles elements at top of page on initial load)
    const checkInitialIntersection = () => {
      if (!ref.current) return;
      const rect = ref.current.getBoundingClientRect();
      const windowHeight = window.innerHeight || document.documentElement.clientHeight;
      // Element is in view if it's at least partially visible
    ...
    // If already in view, don't bother with observer
    if (checkInitialIntersection()) return;
